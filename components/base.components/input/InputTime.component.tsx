...
export const InputTimePickerComponent: React.FC<InputTimePickerProps> = ({
  ...
  return (
    <div className="scale-90 md:scale-100 absolute z-50 top-8 md:top-full -left-4 md:left-0 mt-1 w-max bg-white border rounded-[6px] p-2 shadow flex gap-2">
      <div className="max-h-40 overflow-y-auto bg-white rounded-[6px] input-scroll">
        {timeSlots.map((time) => (
          <div
            ...
            onMouseUp={()=>{handleSelect('h',time.split(':')[0]); handleSelect('m',time.split(':')[1])}}
          >{time}</div>
        ))}
      </div>
